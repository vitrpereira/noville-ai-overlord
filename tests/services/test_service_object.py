import pytest
from app.services.service_object import ServiceObject
...
class InvalidErrorService(ServiceObject):
    def call(self):
        self._errors.append("Invalid error format")  # Not a dictionary
        return None
...
def test_invalid_error_format():
    ...
    assert "str" in str(exc_info.value)  # Should mention the invalid type
